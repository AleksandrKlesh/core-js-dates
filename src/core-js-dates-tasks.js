...
function getDayName(date) {
  ...
  return days[timeStamp.getUTCDay()];
}
...
function getNextFriday(date) {
  ...
  const index = days[date.toString().split(' ')[0]];
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const milliseconds = Date.parse(dateEnd) - Date.parse(dateStart);
  return Math.ceil(milliseconds / (1000 * 60 * 60 * 24)) + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const isIncluded = startDate <= givenDate && givenDate <= endDate;
  return isIncluded;
}
...
function formatDate(date) {
  ...
  return `${dateStr.join('/')}, ${timeStr.join(':')} ${timeOfDay}`;
}
...
function getWeekNumberByDate(date) {
  ...
  const dayUTC = dateUTC.getUTCDay() || 7;
  dateUTC.setUTCDate(dateUTC.getUTCDate() + 4 - dayUTC);

  const firstJan = new Date(Date.UTC(dateUTC.getUTCFullYear(), 0, 1));

  const week = Math.ceil(((dateUTC - firstJan) / milInDay + 1) / 7);
...
function getNextFridayThe13th(date) {
  ...
  while (occurance.getDay() !== friday) {
    occurance = new Date(occurance.getFullYear(), occurance.getMonth() + 1, 13);
  }
...
function isLeapYear(date) {
  const year = date.getFullYear();
  const isDivisibleBy4 = year % 4 === 0 && year % 100 !== 0;
  const isDivisibleBy400 = year % 400 === 0;
  return isDivisibleBy4 || isDivisibleBy400;
}
